...
namespace OpenPhotoViewer.UI.Controls.ImageBox
{
    ...
    public class ImageBox : Control
    {
        ...
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        [Obsolete(
            "This property is deprecated and will be removed in a future version of the component. Implementors should use the SizeMode property instead.")]
        ...
        public virtual int Zoom
        {
            ...
            set
            {
                this.SetZoom(value, value > this.Zoom ? ImageBoxZoomActions.ZoomIn : ImageBoxZoomActions.ZoomOut,
                    ImageBoxActionSources.Unknown);
            }
        }
        ...
        private Point RelativeCenterPoint
        {
            get
            {
                return new Point((this.ScaledImageWidth - this.ClientSize.Width) / 2,
                    (this.ScaledImageHeight - this.ClientSize.Height) / 2);
            }
        }
        ...
        public virtual Rectangle GetImageViewPort()
        {
            ...
            if (!this.ViewSize.IsEmpty)
            {
                ...
                if (!hScroll &&
                    !vScroll) // if no scrolling is present, tinker the view port so that the image and any applicable borders all fit inside
                {
                ...
                if (this.SizeMode != ImageBoxSizeMode.Stretch)
                {
                    if (this.AutoCenter)
                    {
                        ...
                        x = !hScroll
                            ? (innerRectangle.Width - (this.ScaledImageWidth + this.Padding.Horizontal)) / 2
                            : 0;
                        y = !vScroll
                            ? (innerRectangle.Height - (this.ScaledImageHeight + this.Padding.Vertical)) / 2
                            : 0;
                    ...
                    height = Math.Min(this.ScaledImageHeight - Math.Abs(this.AutoScrollPosition.Y),
        ...
        {
            return new Rectangle((int)(source.Left * this.ZoomFactor), (int)(source.Top * this.ZoomFactor),
                (int)(source.Width * this.ZoomFactor), (int)(source.Height * this.ZoomFactor));
        }
        ...
        {
            return new RectangleF((float)(source.Left * this.ZoomFactor), (float)(source.Top * this.ZoomFactor),
                (float)(source.Width * this.ZoomFactor), (float)(source.Height * this.ZoomFactor));
        }
        ...
        public Image GetSelectedImage()
        {
            ...
            if (!this.SelectionRegion.IsEmpty)
            {
                ...
                rect = this.FitRectangle(new Rectangle((int)this.SelectionRegion.X, (int)this.SelectionRegion.Y,
                    (int)this.SelectionRegion.Width, (int)this.SelectionRegion.Height));
        ...
        protected virtual void DrawDropShadow(Graphics g, Rectangle viewPort)
        {
            ...
            rightEdge = new Rectangle(viewPort.Right + 1, viewPort.Top + this.DropShadowSize, this.DropShadowSize,
                viewPort.Height);
            bottomEdge = new Rectangle(viewPort.Left + this.DropShadowSize, viewPort.Bottom + 1, viewPort.Width + 1,
                this.DropShadowSize);
        ...
        protected virtual void DrawImageBorder(Graphics graphics)
        {
            if (this.ImageBorderStyle != ImageBoxBorderStyle.None)
            {
                ...
                graphics.SetClip(this
                    .GetInsideViewPort()); // make sure the image border doesn't overwrite the control border
        ...
        /// <param name="bounds">The <see cref="Rectangle"/> that represents the bounds of the text.</param>
        protected void DrawLabel(Graphics graphics, string text, Font font, Color foreColor, Color backColor,
            Rectangle bounds)
        {
        ...
        /// <param name="bounds">The <see cref="Rectangle"/> that represents the bounds of the text.</param>
        protected void DrawLabel(Graphics graphics, string text, Font font, Color foreColor, Color backColor,
            ContentAlignment textAlign, Rectangle bounds)
        {
        ...
        protected virtual void DrawLabel(Graphics graphics, string text, Font font, Color foreColor, Color backColor,
            ContentAlignment textAlign, Rectangle bounds, bool scaleText)
        {
        ...
        protected virtual void DrawLabel(Graphics graphics, string text, Font font, Color foreColor, Color backColor,
            ContentAlignment textAlign, Rectangle bounds, bool scaleText, Padding padding)
        {
            ...
            if (padding.Horizontal != 0 || padding.Vertical != 0)
            {
                ...
                if (backColor != Color.Empty && backColor.A > 0)
                {
                    ...
                    {
                        graphics.FillRectangle(brush, x - padding.Left, y - padding.Top, width + padding.Horizontal,
                            height + padding.Vertical);
                    }
                }
        ...
        protected virtual void DrawText(PaintEventArgs e)
        {
            ...
            bounds = this.TextDisplayMode == ImageBoxGridDisplayMode.Client
                ? this.GetInsideViewPort()
                : this.GetImageViewPort();

            this.DrawLabel(e.Graphics, this.Text, this.Font, this.ForeColor, this.TextBackColor, this.TextAlign, bounds,
                this.ScaleText, this.TextPadding);
        }
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (!doNotProcessClick && this.AllowZoom && this.AllowClickZoom && !this.IsPanning &&
        ...
        protected virtual void OnSelected(EventArgs e)
        {
            ...
            switch (this.SelectionMode)
            {
                case ImageBoxSelectionMode.Zoom:
                    if (this.SelectionRegion.Width > _selectionDeadZone &&
                        this.SelectionRegion.Height > _selectionDeadZone)
                    {
        ...
        {
            this.PerformZoom(isZoomIn ? ImageBoxZoomActions.ZoomIn : ImageBoxZoomActions.ZoomOut,
                ImageBoxActionSources.User, true, cursorPosition);
        }
        ...
        protected virtual void ProcessPanning(MouseEventArgs e)
        {
            if (this.AutoPan && !this.ViewSize.IsEmpty && this.SelectionMode == ImageBoxSelectionMode.None)
            {
                ...
                if (!this.IsPanning && (this.ScaledImageWidth > clientSize.Width ||
                                        this.ScaledImageHeight > clientSize.Height))
                {
        ...
        protected virtual void ProcessScrollingShortcuts(KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Left:
                    this.AdjustScroll(
                        -(e.Modifiers == Keys.None
                            ...
                            : this.HorizontalScroll.LargeChange), 0);
                    break;
                ...
                case Keys.Up:
                    this.AdjustScroll(0,
                        -(e.Modifiers == Keys.None
                            ? this.VerticalScroll.SmallChange
                            : this.VerticalScroll.LargeChange));
                    break;

                case Keys.Down:
                    this.AdjustScroll(0,
                        e.Modifiers == Keys.None ? this.VerticalScroll.SmallChange : this.VerticalScroll.LargeChange);
                    break;
            }
        }
        ...
        {
            if (this.SelectionMode != ImageBoxSelectionMode.None && e.Button == MouseButtons.Left &&
                !this.WasDragCancelled)
            {
                if (!this.IsSelecting)
                {
                    this.StartDrag(e);
        ...
        private void PerformActualSize(ImageBoxActionSources source)
        {
            this.SizeMode = ImageBoxSizeMode.Normal;
            this.SetZoom(100,
                ImageBoxZoomActions.ActualSize |
                (this.Zoom < 100 ? ImageBoxZoomActions.ZoomIn : ImageBoxZoomActions.ZoomOut), source);
        }
